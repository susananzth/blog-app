<x-modal title="{{ __('Edit Post') }}" wire:model="updatePost" focusable>
    <form class="mt-6 space-y-6" method="POST">
        ...
        <div>
            ...
            <x-input.text id="title" class="block mt-1 w-full" type="text"
                name="title" :value="$title" wire:model="title"
        ...
        <div>
            ...
            <x-input.textarea id="body" name="body" :value="$body" 
        ...
        <div>
            ...
            <x-input.select id="status" class="block mt-1 w-full" 
                ...
                <option value="false" {{ ($status == false) ? 'selected' : '' }}>{{ __('Created') }}</option>
                <option value="true" {{ ($status == true) ? 'selected' : '' }}>{{ __('Published') }}</option>
            </x-input.select>
